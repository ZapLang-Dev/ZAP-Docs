...
function DocsPage() {
  ...
  const lessonContent: Record<string, Lesson> = {
    ...
    installation: { intro: "Zap is distributed as a standalone native executable. No separate language runtime is required. Download the archive for your system, extract it, and add the zap command to your PATH.", blocks: [["Supported targets", "Official releases are published for Linux x86_64, Windows x86_64, and macOS ARM64. Select the asset that matches your computer's architecture.", "zap --version"], ["Downloads (v2.0.4)", "Download the archive for your platform: [Linux x86_64](https://github.com/hidecard/zap/releases/download/v2.0.4/zap-2.0.4-linux-x86_64.tar.gz), [macOS ARM64](https://github.com/hidecard/zap/releases/download/v2.0.4/zap-2.0.4-macos-arm64.tar.gz), or [Windows x86_64](https://github.com/hidecard/zap/releases/download/v2.0.4/zap-2.0.4-windows-x86_64.zip). View the [official changelog](https://github.com/hidecard/zap/blob/master/CHANGELOG_EN.md) for recent updates."], ["Linux & macOS", "Extract the .tar.gz archive and run the included install.sh script. On macOS, you may need to grant execute permission to the installer first.", "tar -xzf zap-2.0.4-linux-x86_64.tar.gz\nbash install.sh"], ["Windows", "Download the .zip archive, extract it to a folder like C:\\Zap, and run install_windows.bat from Command Prompt as a normal user.", "cd C:\\Zap\ninstall_windows.bat"]] },
